...
<div class="cart-items">
	<div class="container">
			   <?php
						...
						if(!(isset($_SESSION["sCustId"])))
						{	
							echo '<script> alert("Please Login to View your Orders!!"); 
						...
						else
						{
							...
							$result = loadCustOrder($custId);
							$GLOBALS['totalItem'] = $result->num_rows;
							
							echo '<h2><span style = "margin-right:5%;">My Orders ('.$GLOBALS['totalItem'].')</span><span> <a href = "cart.php">View Cart</a></span></h2>';
							...
							if ($result->num_rows > 0) 
								{
									while($row = $result->fetch_assoc()) 
									{
										...
										if($row['orderConfirm'] != NULL && $row['orderConfirm'] == 1)
										{
											$oStatus = "Order Approved";
										}
										else if($row['orderConfirm'] != NULL && $row['orderConfirm'] == 0)
										{
											$oStatus = "Order Rejected";
										}
										else
										{
											$oStatus = "Pending for Approval";
										}
										$oDate = $row['orderDate'];
